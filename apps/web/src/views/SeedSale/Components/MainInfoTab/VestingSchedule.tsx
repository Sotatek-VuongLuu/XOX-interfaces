...
const SaleItem = ({
  ...
  item: IVestingTime
  index: number
  handleClaim: (remainning: any) => void
    ...
    <div className={`sale_item sale_item_${index}`}>
      ...
      <div className="sale-container-bottom ">
        ...
        <div className="sale-schedule-item flex-col">
          ...
          <p>
            <span>
              <CountDown startTime={itemVesting.startTime[count]} setCount={setCount} count={count} numberOfClaims={numberOfClaims} />
            </span>
          </p>
        </div>
        <div className="sale-schedule-item no-padding">
          <CustomButtom
            type="button"
            onClick={() => handleClaim(itemVesting.yourCurrentXOX)}
...
const CountDown = ({ startTime, setCount, count, numberOfClaims }: Props) => {
  const { t } = useTranslation()
  ...
  const handleCountDown = () => {
    ...
    if (timeStampAtNow >= startTime) {
      ...
      count < numberOfClaims && setCount(count + 1)
    }
  }
...
function VestingSchedule({
  ...
  useEffect(() => {
    setDataSeedSaleSchedule(dataSeedSale);
    setDataPartnerSaleSchedule(dataPartnerSale);
  }, [dataSeedSale, dataPartnerSale, handleClaimSeedSale, handleClaimPartnerSale, handleGetDataVesting])
    ...
    return (
      <>
        <SaleItem
          ...
          item={dataSeedSale}
          index={1}
          handleClaim={handleClaimSeedSale}
          handleGetDataVesting={handleGetDataVesting}
          numberOfClaims={5}
        />
        <SaleItem
          // eslint-disable-next-line react/no-array-index-key
          item={dataPartnerSale}
          index={2}
          handleClaim={handleClaimPartnerSale}
          handleGetDataVesting={handleGetDataVesting}
          numberOfClaims={18}
        />
      </>
    )
  }, [handleClaimSeedSale, handleClaimPartnerSale, handleGetDataVesting, dataSeedSaleSchedule, dataPartnerSaleSchedule])
      ...
      {account && (
        <>
          <div className="total_vested">
            ...
            {Number(
              new BigNumber(dataSeedSale?.remaining)
                .plus(dataPartnerSale?.remaining)
                .toFixed(2),
            ).toLocaleString()}
          </div>
          <Content>
